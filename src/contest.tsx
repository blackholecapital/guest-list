      ...
      <section className="contest-form-panel">
        {success ? <div className="contest-success"><span>✓</span><p className="eyebrow">Entry received</p><h2>You’re officially in the running.</h2><p>Our team will review your submission and follow up by text or email with your contest details.</p><button className="secondary-button" onClick={() => setSuccess(false)}>Submit another entry</button></div> : <>
          ...
          <form className="contest-form" onSubmit={submit}>
            ...
            <label className="contest-upload">Photos <small>Add up to 3 clear photos · JPG, PNG, or WebP · 8 MB each</small><input name="photos" type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={e => { const files = Array.from(e.target.files || []).slice(0, 3); if ((e.target.files?.length || 0) > 3) e.target.value = ""; setPhotoNames(files.map(f => f.name)); }} /><span>{photoNames.length ? `${photoNames.length} photo${photoNames.length === 1 ? "" : "s"} selected` : "Choose photos"}</span></label>
            <label className="contest-check"><input name="ageConfirmed" type="checkbox" value="yes" required /><span>I confirm that I am at least 21 years old and that any photos I submit are of me.</span></label>
